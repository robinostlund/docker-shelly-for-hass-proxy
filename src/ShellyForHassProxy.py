...
def main():
  ...
  parser.add_argument('--hass-port', dest='udp_port', type=int, action='store', help="Home Assistant Port", required=True)
  parser.add_argument('--proxy-ip', dest="udp_ip", type=str, help='Shelly For Hass Proxy IP', required=True)
  parser.add_argument('--proxy-port', dest='udp_port', type=int, action='store', help="Shelly For Hass Proxy Port", required=True)
